...
def task_4_update_customer(con):
    ...
    """
    with con.cursor() as remote:
        remote.execute("UPDATE customers SET customername = 'Johnny Depp' WHERE customerid = 1")
...
def task_5_delete_the_last_customer(con) -> None:
    ...
    """
    with con.cursor() as remote:
        remote.execute("DELETE FROM customers WHERE customerid = (SELECT MAX(customerid) FROM customers)")
...
def task_6_list_all_supplier_countries(cur) -> list:
    ...
    """
    cur.execute("SELECT country FROM suppliers WHERE country IS NOT NULL;")
    return cur.fetchall()
...
def task_7_list_supplier_countries_in_desc_order(cur) -> list:
    ...
    """
    cur.execute("SELECT country FROM suppliers WHERE country IS NOT NULL ORDER BY country DESC ;")
    return cur.fetchall()
...
def task_8_count_customers_by_city(cur):
    ...
    """
    cur.execute("""SELECT city, COUNT(DISTINCT city) FROM customers GROUP BY city;""")
    return cur.fetchall()
...
def task_10_list_first_10_customers(cur):
    ...
    Results: 10 records
    """
    cur.execute("SELECT * FROM customers FETCH FIRST 10 ROWS ONLY;")
    return cur.fetchall()
...
def task_12_list_suppliers_from_specified_countries(cur):
    ...
    Returns: 8 records
    """
    cur.execute("SELECT * FROM suppliers WHERE country IN ('USA', 'UK', 'Japan') ORDER BY supplierid;")
    return cur.fetchall()
...
def task_13_list_products_from_sweden_suppliers(cur):
    ...
    (SELECT suppliers.supplierid FROM suppliers WHERE country = 'Sweden')""")
    return cur.fetchall()
